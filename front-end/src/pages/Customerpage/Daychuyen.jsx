  ...
  useEffect(() => {
    ...
    const fetchNecklaces = async () => {
      ...
      try {
        ...
        if (ignore) return;
        setError(
          err?.response?.data?.message || err.message || "Không thể tải danh sách dây chuyền."
        );
      } finally {
          ...
          <div className="relative mx-auto flex max-w-6xl flex-col gap-6 px-6 py-14 sm:flex-row sm:items-center sm:justify-between sm:py-16 lg:px-10">
            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-[0.36em] text-[#8a6f58]">
                Bộ sưu tập Dây chuyền
              </p>
              <h1 className="text-3xl font-bold leading-tight text-[#2f241a] sm:text-4xl">
                Phụ kiện nhỏ, điểm nhấn lớn
              </h1>
              <p className="max-w-xl text-sm text-[#5f4a38] sm:text-base">
                Những mẫu dây chuyền dễ đeo mỗi ngày, phù hợp nhiều hoàn cảnh – đi làm, dạo phố hay
                dự tiệc – giúp bạn luôn nổi bật mà không cần quá cầu kỳ.
              </p>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="space-y-1">
                <p className="text-lg font-semibold uppercase tracking-[0.2em] text-[#2f241a]">
                  Dây chuyền cao cấp tại Jewelux
                </p>
                <p className="text-xs text-[#7b6654]">
                  {filteredNecklaces.length} sản phẩm phù hợp lựa chọn của bạn
                </p>
              </div>
