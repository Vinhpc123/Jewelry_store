...
export default function NecklacesPage() {
  ...
  useEffect(() => {
    ...
    const fetchNecklaces = async () => {
      ...
      setError(null);
      try {
        const res = await instance.get("/api/jewelry", { params: { category: "Dây chuyền" } });
        if (ignore) return;
        const items = Array.isArray(res.data) ? res.data : [];
        setNecklaces(items);
      } catch (err) {
        if (ignore) return;
        setError(err?.response?.data?.message || err.message || "Không thể tải danh sách dây chuyền.");
      } finally {
  ...
  const filteredNecklaces = useMemo(() => {
    ...
    let list = necklaces;
    if (range?.min !== undefined) list = list.filter((item) => Number(item.price) >= range.min);
    if (range?.max !== undefined) list = list.filter((item) => Number(item.price) <= range.max);
        ...
        <section className="relative isolate overflow-hidden bg-[#ece8e1]">
          <div className="relative mx-auto flex max-w-6xl flex-col gap-6 px-6 py-14 sm:flex-row sm:items-center sm:justify-between sm:py-16 lg:px-10">
            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-[0.36em] text-[#8a6f58]">Bộ sưu tập Dây chuyền</p>
              <h1 className="text-3xl font-bold leading-tight text-[#2f241a] sm:text-4xl">Phụ kiện nhỏ, điểm nhấn lớn</h1>
              <p className="max-w-xl text-sm text-[#5f4a38] sm:text-base">
                Những mẫu dây chuyền dễ đeo mỗi ngày, phù hợp nhiều hoàn cảnh – đi làm, dạo phố hay dự tiệc – giúp bạn luôn nổi bật mà không cần quá cầu kỳ.
              </p>
            ...
            <div className="relative isolate aspect-[4/3] w-full max-w-md overflow-hidden rounded-3xl bg-white/60 shadow-lg ring-1 ring-[#e8d9c7]">
              <img src="/daychuyen.webp" alt="Necklace collection" className="h-full w-full object-cover" />
        ...
        <section className="mx-auto max-w-6xl px-5 pb-16 pt-10 sm:px-6 lg:px-10">
          <header className="flex flex-col gap-4 border-b border-[#eadfce] pb-6 text-sm text-[#6d5a48]">
            <div className="flex items-center gap-2 text-xs uppercase tracking-[0.24em]">
              ...
              <span>/</span>
              <span className="text-[#2f241a] font-semibold">Dây chuyền</span>
            </div>
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="space-y-1">
                <p className="text-lg font-semibold uppercase tracking-[0.2em] text-[#2f241a]">Dây chuyền cao cấp tại Jewelux</p>
                <p className="text-xs text-[#7b6654]">{filteredNecklaces.length} sản phẩm phù hợp lựa chọn của bạn</p>
              </div>
                  ...
                  {sortOptions.map((opt) => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs font-semibold transition ${
                        sortBy === opt.id ? "bg-[#2f241a] text-white" : "text-[#6d5a48] hover:bg-[#f4e7d8]"
                      }`}
                    >
                      {opt.label}
                    </button>
            ...
            {priceRanges.map((range) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-xs font-semibold transition ring-1 ring-[#e8d9c7] ${
                  priceFilter === range.id ? "bg-[#2f241a] text-white" : "bg-white text-[#6d5a48] hover:bg-[#f4e7d8]"
                }`}
              >
                {range.label}
              </button>
          ...
          {loading ? (
            <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {Array.from({ length: 6 }).map((_, idx) => (
                <div key={idx} className="animate-pulse rounded-3xl bg-white/80 p-5 shadow-sm ring-1 ring-[#eadfce]">
          ...
          {!loading && !error && filteredNecklaces.length === 0 ? (
          ...
          {!loading && !error && filteredNecklaces.length > 0 ? (
            <div className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
              {filteredNecklaces.map((item) => {
                const name = item.title || item.name || "Dây chuyền chưa đặt tên";
