// Bong tai (earrings) product page
...
const priceRanges = [
  ...
  { id: "under2", label: "Dưới 2.000.000", max: 2_000_000 },
  { id: "2to5", label: "2.000.000 - 5.000.000", min: 2_000_000, max: 5_000_000 },
  { id: "over5", label: "Trên 5.000.000", min: 5_000_000 },
];
        ...
        <section className="relative isolate overflow-hidden bg-[#ece8e1]">
          <div className="relative mx-auto flex max-w-6xl flex-col gap-6 px-6 py-14 sm:flex-row sm:items-center sm:justify-between sm:py-16 lg:px-10">
            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-[0.32em] text-[#8f735c]">
                Bộ sưu tập bông tai
              </p>
        ...
        <section className="mx-auto max-w-6xl px-5 pb-16 pt-10 sm:px-6 lg:px-10">
          <header className="flex flex-col gap-4 border-b border-[#eadfce] pb-6 text-sm text-[#6d5a48]">
            <div className="flex items-center gap-2 text-xs uppercase tracking-[0.24em]">
              ...
              <span>/</span>
              <span className="text-[#2f241a] font-semibold">Bông tai</span>
            </div>
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="space-y-1">
                <p className="text-lg font-semibold uppercase tracking-[0.2em] text-[#2f241a]">
                  Bông tai cao cấp tại Jewelux
                </p>
                <p className="text-xs text-[#7b6654]">
                  {filteredEarrings.length} sản phẩm phù hợp lựa chọn của bạn
                </p>
              </div>
          ...
          {!loading && !error && filteredEarrings.length === 0 ? (
          ...
          {!loading && !error && filteredEarrings.length > 0 ? (
            <div className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
              {filteredEarrings.map((item) => {
                const name = item.title || item.name || "Bông tai chưa đặt tên";
